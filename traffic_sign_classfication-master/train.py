...
def args_parse():
    ...
    ap.add_argument("-train", "--data_train", required=True, default="./data/train",help="path to input data_train")
    ap.add_argument("-test", "--data_test", required=True, default="./data/test",help="path to input data_test")
    ap.add_argument("-m", "--model", required=True, default="traffic_sign.model",help='path to output the model')
...
def train(idg, X_train, X_test, y_train, y_test, args):
    ...
    '''loss="categorical_squared_hinge categorical_crossentropy"'''
    #训练网络参数
    ...
    plt.title("Accuracy on traffic-sign classifier")
    plt.xlabel("Epoch#")
    plt.ylabel("Accuracy")
    ...
    #pinghuaquxian
    x=list(range(1,101))
    plt.plot(np.arange(0, N),smooth_curve(H.history['acc']),'b',label = 'smoothed traing acc')
    plt.plot(np.arange(0, N),smooth_curve(H.history['val_acc']),'r',label='smoothed val acc')
